...
class QuestionGenerator:
    ...
    def generate(
        ...
        """"  If use_evaluator is True then QA pairs will be ranked and filtered based on their quality. answer_style
    ...
    def generate_questions_from_inputs(self, qg_inputs: List) -> List[str]:
        """ Model Input as "answer_token <answer text> context_token <context text>"""
        generated_questions = []
    ...
    def _split_into_segments(self, text: str) -> List[str]:
        """Splits a long text into segments."""
    ...
    def sense2vec_get_words(self,word,s2v,question):
      output = []
      print ("word ",word)
      try:
    ...
    def mmr(self,doc_embedding, word_embeddings, words, top_n):
      ...
      for _ in range(top_n - 1):
          ...
          # Calculate MMR
          mmr = (0.2) * candidate_similarities - (1-0.2) * target_similarities.reshape(-1, 1)
        ...
        for doc, sentence in zip(docs, sentences):
            ...
            if entities:
                ...
                for entity in entities:
                    ...
                    distractors = self.sense2vec_get_words(str(entity).capitalize(),self.s2v,sentence)
                    if len(distractors) == 0:
                      answers = self._get_MC_answers(entity, distractors, docs)
                      inputs_from_text.append(qg_input)
                      answers_from_text.append(answers)

                    distractors_new = [str(entity).capitalize()]
                    distractors_new.extend(distractors)

                    embedding_sentence = sentence+ " "+str(entity).capitalize()
                    ...
                    filtered_keywords = self.mmr(keyword_embedding, distractor_embeddings,distractors_new,max_keywords)
                    final = [str(entity).capitalize()]
                    for wrd in filtered_keywords:
                      if wrd.lower() !=str(entity).lower():
                        final.append(wrd.capitalize())
                    final = final[1:]

                    answers = self._get_MC_answers(entity, final, docs)
                    inputs_from_text.append(qg_input)
                    answers_from_text.append(answers)

        return inputs_from_text, answers_from_text

    def _get_MC_answers(self, correct_answer, dist, docs) -> List[Mapping[str, Any]]:
        """Finds a set of alternative answers for a multiple-choice question."""
        ...
        if len(dist) == 0:
          ...
          for x in dist:
              final_choices.append({"answer": x, "correct": False})
    ...
    def _generate_question(self, qg_input: str) -> str:
        """Takes qg_input which is the concatenated answer and context, and uses it to generate a question sentence."""
    ...
    def _get_ranked_qa_pairs(
        ...
        if num_questions > len(scores):
            ...
            print((
                f"Was only able to generate {num_questions} questions.",
...
class QAEvaluator:
    ...
    def _encode_qa(self, question: str, answer: str) -> torch.tensor:
        """Concatenates a question and answer, and then tokenizes them."""
